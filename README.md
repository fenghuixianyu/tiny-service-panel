...
它默认使用 **systemd socket activation**：开机自启的是 socket 监听，Python 后端只有在访问面板/API 时才会被 systemd 拉起，空闲 10 分钟后自动退出。

## 新服务器最快安装

如果要在手机或电脑上直接用 `http://服务器IP:8765` 访问，推荐在新服务器执行：
...
- 默认会启用 `tiny-service-panel.socket` 开机自启；后端服务不是常驻进程，只在访问时启动，空闲 10 分钟后退出。
...
- 支持收藏和备注
- 支持隐藏常见 systemd 噪音项，并可单独查看 systemd 基础、登录/会话、设备/挂载、系统定时器等屏蔽区
- 仪表盘显示内存、Swap、Load、根分区、运行时间、重启状态和可收纳的磁盘分区
- 支持异常服务置顶，方便优先处理 failed / activating / auto-restart 服务
- 手机端使用卡片布局，保留搜索、类型、排序、运行状态和自启状态筛选
...
如果公网安装时不传 `AUTH_PASSWORD`，脚本会自动生成一个随机密码并只打印一次：
...
因此服务器重启后会自动恢复监听。`tiny-service-panel.service` 不会被设置为常驻自启，它会在访问面板时按需启动，空闲 10 分钟后退出。
...
如果你已经设置密码并确实要公网访问，需要让 socket 监听所有网卡：
...
登录页勾选“记住此设备”后，会在当前浏览器保存 HttpOnly Cookie，默认 30 天内不用重复输入密码。换浏览器、清理 Cookie、点击退出登录、重装并更换密码后，都需要重新登录。可以修改记住时间：
...
默认仍会隐藏屏蔽区，减少服务列表噪音。你也可以通过“全部分区 / 常用区 / 屏蔽区”筛选单独查看。
...
仪表盘会显示系统是否建议重启，并提供“重启服务器”按钮。点击后需要二次确认并输入 `REBOOT`，防止误触。
...
此面板默认以 root 运行，能够执行 `systemctl start/stop/restart`。即使加了密码，也建议优先放在内网、VPN、Cloudflare Access、Tailscale/WireGuard 或 SSH 隧道后面；如果必须公网开放，请使用强密码并只放行可信来源 IP。
